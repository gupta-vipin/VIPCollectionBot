...
def handle_empty_arg(*, required_params, error_message='An argument was missing', parse_mode=None):
    ...
    def restricted_func(bot, update, **kwargs):
        ...
        if authorized_user(user_id):
            ...
            update.effective_message.reply_text(
                'You must register first to use this command. Write `/register`',
                parse_mode='markdown',
                quote=False
            )
...
def private_chat_only(func):
    ...
    def deco_func(bot, update, **kwargs):
        ...
        if chat_type == telegram.Chat.PRIVATE:
            ...
            update.effective_message.reply_text(
                'The command works only in private conversation with @VIPCollectionbot.\n'
                'Click the username to start a conversation with him.',
                parse_mode='markdown',
                quote=False
            )
