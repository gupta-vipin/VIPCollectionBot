...
def serie(bot, update, chat_data, args):
    if not args:
        bot.send_message(
            ...
            text='You missed giving me the name of the series. `/serie <serie>`',
            parse_mode='markdown',
        )
        return
    ...
    if r.status_code != 200:
        bot.send_message(
            chat_id=update.message.chat_id,
            text=f"I did not find information on imdb about '{serie_query}'. Is the name spelled correctly?",
        )
        return
    ...
    try:
        ...
        bot.send_message(
            chat_id=update.message.chat_id,
            text=f"I did not find results on imdb about '{serie_query}'",
        )
        return
