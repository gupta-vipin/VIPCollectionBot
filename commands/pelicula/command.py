...
def buscar_peli(bot, update, chat_data, **kwargs):
    ...
    if not pelicula:
        bot.send_message(
            ...
            text='I need you to pass me a movie. `/pelicula <name>`',  # Todo: Add deeplink with example
            parse_mode='markdown',
        )
        return
    ...
    try:
        ...
        if not movie:
            bot.send_message(
                chat_id=update.message.chat_id,
                text='I did not find info about %s' % pelicula_query,
            )
            return
        ...
        bot.send_message(
            chat_id=update.message.chat_id,
            text='I am resting now, try after nap',
            parse_mode='markdown',
        )
