...
@handle_empty_arg(required_params=('args',), error_message='And the url of the video? `/yttomp3 <url>`',
                  parse_mode='markdown')
def youtube_to_mp3(bot, update, args):
    ...
    try:
        ...
        update.message.reply_text(f'✅ File downloaded. Sending...')
        ...
        if file:
            ...
            update.message.reply_text(f'💬 Tip: You can use [VLC]({VLC_LINK}) to play the audio 🎶',
                                      parse_mode='markdown', disable_web_page_preview=True)
            file.close()
            logger.info('File sent successfully')
    except NetworkError:
        logger.error('A network error occurred.', exc_info=True)
        update.message.reply_text(text='🚀 There are connection problems right now. Try later ..')
        send_message_to_admin(bot, f'Error sending {video_url}, {filename}')
